...
def get_top_players():
    top_players_array = []
    for player in top_players():
        top_players_array.append({
            'country': player['country'].decode("utf-8"),
            'name': player['name'], 
            'nickname': player['nickname'].decode("utf-8"), 
            'rating': float(player['rating'].decode("utf-8")), 
            'maps_played': int(player['maps-played'].decode("utf-8"))
        }) 
